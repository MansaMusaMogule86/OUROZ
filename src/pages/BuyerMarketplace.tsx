...
const BuyerMarketplace: React.FC<MarketplaceProps> = ({ language, onNavigate, userId }) => {
    ...
    const { products, loading, pagination, setParams, fetchPage } = useSupplierProducts(
        ...
        {
            page: 1, // Start with page 1 to prevent circular dependency warning
            limit: 20,
...
// Sub-components
// ⚡ Bolt: Added React.memo and local debounced state to prevent full page re-renders on every keystroke
// Performance: Eliminates layout thrashing by updating parent state only when typing stops
const MarketplaceHeader: React.FC<{
    search: string;
    onSearchChange: (s: string) => void;
    onNavigate: (path: string) => void;
}> = React.memo(({ search, onSearchChange, onNavigate }) => {
    ...
    // Safely sync from parent *only* when cleared externally (e.g., "Clear All Filters")
    // Prevents race conditions that overwrite user typing
    useEffect(() => {
        if (search === '') {
            setInputValue('');
        }
